---
title: Интеграция с IDE, AI и MCP
description: Export, Live Import, AI-ассистенты и MCP
published: true
...
Встроенный редактор скриптов EyeAuras рассчитан в основном на небольшие проекты - простой кликер, небольшой бот, какая-то вспомогательная утилита, ну и, разумеется, использование прикладных скриптов в [деревьях поведения](/ru/behavior-trees/scripting) и [макросах](/ru/macros/getting-started). 

В какой-то момент, его возможностей просто перестает хватать - вы хотите разложить классы удобно по папочкам, навигироваться по коду с помощью умного поиска, написать юнит-тесты и пользоваться всеми остальными прелестями современной разработки, которые предоставляют IDE типа [JetBrains Rider](https://www.jetbrains.com/rider/), [Visual Studio](https://visualstudio.microsoft.com/) или [Visual Studio Code](https://code.visualstudio.com/). 
...
# Пример
Работать будем с [Hello, World](/ru/scripting/examples/basic/hello-world) - никакой принципиальной разницы нет, все ауры ведут себя примерно одинаково. 
...
- возможность подключать внешние библиотеки - предпочтительнее всего через Embedded Resources
...
- [JetBrains Rider](https://www.jetbrains.com/rider/) - мой личный выбор
...
- очень сильная навигация по C# коду
- хорошая работа с большими `.sln`
- удобные refactoring tools
- нормальная работа с Razor, Blazor и обычными `*.cs`
- хороший баланс между "мощная IDE" и "не мешает работать"

Важный нюанс: сейчас Rider часто по привычке называют "community", но технически речь идет не про отдельную Community Edition. С **24 октября 2024 года** Rider просто стал бесплатным для **non-commercial use**. Официальная информация: [Rider free for non-commercial use](https://blog.jetbrains.com/blog/2024/10/24/webstorm-and-rider-are-now-free-for-non-commercial-use/).
...
- привычная среда для многих .NET-разработчиков
- отличная интеграция с экосистемой Microsoft
- хороший выбор, если вы любите именно "большую" IDE под Windows

Если нужен бесплатный вариант, смотрите [Visual Studio Community](https://visualstudio.microsoft.com/products/visual-studio-community-vs). Для индивидуальных разработчиков она бесплатна, а для организаций там уже есть свои условия лицензии.
...
- легкий и быстрый
- очень удобен, если вы любите editor-first workflow
- отлично чувствует себя в AI-first сценариях
- у него очень зрелая поддержка MCP и agent workflow

Для EyeAuras я бы рекомендовал VS Code в двух случаях:

- если вы уже привыкли к нему и не хотите пересаживаться
- если вам нужен максимально гибкий AI/MCP setup

Но если говорить именно про комфорт работы с большим C# проектом EyeAuras, мой личный выбор все равно - Rider.

# AI-ассистенты: пользоваться ими уже просто выгодно

На практике в 2026 году писать код совсем без AI обычно просто менее выгодно:

- дольше искать API
- дольше собирать boilerplate
- дольше проверять идеи
- дольше разбираться в чужом коде

То есть вопрос уже обычно не в том, "нужен ли AI", а в том, **какой именно AI лучше встроить в рабочий процесс**.

## Что использовать в разных IDE

### Rider

Для Rider самый очевидный вариант - [JetBrains AI Assistant](https://www.jetbrains.com/help/ai-assistant/about-ai-assistant.html).

Хорошая новость в том, что у JetBrains AI Assistant уже есть прямая интеграция с [Codex](https://www.jetbrains.com/help/ai-assistant/codex-agent.html), и это как раз мой текущий личный выбор для EyeAuras: **Rider + AI Assistant + Codex**.

Почему именно так:

- все живет прямо внутри IDE
- есть agent mode
- есть поддержка MCP
- хорошее соотношение цена/качество
- удобно для длинных задач, а не только для "допиши одну строчку"

### Visual Studio

Для Visual Studio самый прямой и практичный вариант - [GitHub Copilot](https://visualstudio.microsoft.com/github-copilot/).
...
- [GitHub Copilot](https://code.visualstudio.com/docs/editor/artificial-intelligence)
- [Codex IDE extension / Codex workflow](https://openai.com/index/introducing-the-codex-app/)

Плюс у VS Code сейчас очень сильная и зрелая поддержка [MCP](https://code.visualstudio.com/docs/copilot/chat/mcp-servers), что делает его очень удобным хостом для AI-инструментов.
...
Это не единственный рабочий вариант, но сейчас это, на мой взгляд, один из самых удобных и сильных сценариев по соотношению качества, цены и количества ручной возни.
...
- открывать нужные файлы
- вызывать специальные инструменты
- редактировать код через контролируемые tool calls
- получать дополнительные знания из документации, логов, ресурсов и API
...
- [MCP в JetBrains AI Assistant](https://www.jetbrains.com/help/ai-assistant/mcp.html)
- [MCP в VS Code](https://code.visualstudio.com/docs/copilot/chat/mcp-servers)

# EyeAuras MCP: зачем он особенно полезен
...
- смотреть workspace текущего скрипта
- читать и менять документы в проекте
- искать символы и документацию
- проверять диагностику компиляции
- работать с NuGet-зависимостями

Именно поэтому самый сильный сценарий сейчас выглядит так:

- вы открываете экспортированный `.sln` в IDE
- даете AI доступ к самому проекту
- дополнительно подключаете к нему **EyeAuras MCP**

В результате получается ассистент, который одновременно:

- видит ваш обычный C# проект
- понимает EyeAuras-специфику
- умеет не просто советовать, а реально вносить правки

## Как запустить EyeAuras MCP

1. Откройте в EyeAuras вкладку [AI Assistant](/ru/features/ai-assistant) / `AI`.
2. При необходимости нажмите `Show Settings`.
3. Проверьте поле `MCP Port`.
4. Нажмите `Start MCP`.

По умолчанию порт сейчас `48080`, а endpoint выглядит примерно так:
...
# Export 
Идея проста - ваш C# скрипт экспортируется в виде полноценного C# проекта, который можно даже сбилдить с помощью IDE. Далее вы работаете над ним, используя современные и удобные **вам** средства разработки, а когда все готово - просто импортируете проект обратно в EyeAuras, тем самым получая лучше от обоих миров. Вы все еще можете использовать все встроенные возможности EyeAuras SDK, такие как [работу с изображениями](/ru/scripting/examples/basic/image-search), [симуляцию ввода](/ru/scripting/examples/basic/send-text), [работу с памятью](/ru/scripting/api/memory), создание кастомных интерфейсов на [Blazor](/ru/scripting/blazor-windows/getting-started) и [ImGui](/ru/scripting/imgui/getting-started), ну и все остальное. 

В декабре 2024 появился прототип [экспорта](/ru/changelogs/7930) и на протяжении года он постепенно совершенствовался и разрастался. 
