...
### Что такое default references

`Default references` в EyeAuras это ссылки, которые среда добавляет сама:

- metadata references на сборки
- и `using`-импорты namespace для часто используемых типов

Они собираются не из одного фиксированного списка, а из нескольких источников:

- базовые .NET, Blazor, ReactiveUI, PoeShared и EyeAuras типы, которые считаются типичными для скриптов
- зарегистрированные scripting APIs
- известные entity types из репозитория EyeAuras
- metadata-сборки уже загруженных модулей EyeAuras и PoeShared

Практические последствия:

- если тип уже есть в default references, часто достаточно обычного `using`, без дополнительного `#r`
- default references не стоит считать полностью фиксированным списком на все версии; он может меняться по мере загрузки модулей и развития продукта
- то, что сборка "есть внутри EyeAuras", еще не всегда означает, что нужный namespace уже автоматически импортирован в текущий script

Хорошее правило при ошибках вида "type/namespace not found":

- сначала проверить, не хватает ли просто `using`
- затем решить, нужен ли `#r "assemblyName: ..."` для уже загруженной host-сборки
- для внешней библиотеки предпочесть `#r "nuget: ..."`
- для локальной или приватной DLL использовать `#r "assemblyPath: ..."`

### Когда достаточно `assemblyName`

Если сборку уже использует сама EyeAuras или один из ее модулей, она может уже быть загружена в `AppDomain` / `AssemblyLoadContext`, даже если не входит в default references.

В такой ситуации бывает достаточно простой ссылки по имени:
...
- сборка уже поставляется вместе с EyeAuras или ее модулем
- сборка уже загружена хостом, но не добавлена в default references
- вам не нужен отдельный DLL path и не нужно тянуть внешний пакет из NuGet

Но это не универсальная замена для NuGet:

- если зависимость внешняя и должна нормально восстанавливаться и переноситься вместе с проектом, обычно лучше `#r "nuget: PackageId, Version"`
- если DLL ваша собственная и лежит рядом со script, обычно лучше `#r "assemblyPath: ..."` или embedded resource
...
### Для UI не допускайте утечки исключений и логируйте важные события

Это особенно важно для `Blazor Windows`, `ImGui` и вообще любого UI-кода, который живет дольше одного короткого вызова.

Практическое правило:

- не позволяйте исключениям "вылетать" из `Rx`-подписок, обработчиков событий и `async`-операций
- такие исключения могут уронить не только сам скрипт, но и все приложение целиком
- важные шаги жизненного цикла и все значимые исключения лучше явно писать в `Log`

Почему это важно:

- по логам намного проще понять, в каком состоянии находились окно, overlay, подписки и фоновые операции
- хороший лог часто помогает заметить плохое состояние раньше, чем оно превратится в реальное падение
- `async void`, fire-and-forget задачи и подписки без собственного `try/catch` особенно легко "теряют" исключения
...
- фоновые `Task.Run(...)`
- код внутри `OnInitializedAsync`, `OnAfterFirstRenderAsync` и похожих методов

то безопаснее сразу думать так:

- где именно будет поймано исключение
- что именно попадет в `Log`
- как быстро по логам будет понятно, на каком шаге все сломалось

Для `Razor`-компонентов обычно лучше держать разметку в отдельном `*.razor`-файле и внимательно следить за `namespace`.

Исключение:

- если это реально однофайловый скрипт
- и пользователь явно просит держать все в одном файле

в таком случае можно оставить все в одном месте, но это должно быть осознанным исключением, а не дефолтом.
