...
- Suppress WebView/browser shortcuts such as Ctrl+A, Ctrl+P, and F5 while
  keeping Blazor shortcut handlers active.
...
- `IJsPoeBlazorUtils.SuppressWellKnownBrowserShortcuts(...)` - explicit opt-in
  DOM listener for blocking common browser/WebView default shortcuts. It returns
  an async-disposable handle and does not stop propagation by default.
...
- Browser/WebView shortcuts that conflict with app UI can be suppressed with
  `IJsPoeBlazorUtils.SuppressWellKnownBrowserShortcuts(...)`; add
  `data-poe-browser-shortcuts="allow"` to DOM subtrees that should opt out.
...
1. Inject `IJsPoeBlazorUtils` in the hosted component or shell.
2. Call `SuppressWellKnownBrowserShortcuts()` during component initialization.
3. Store and dispose the returned `JsBrowserShortcutSuppressionRef` with the
   component/window lifetime.
4. Use `JsBrowserShortcutSuppressionOptions` for custom shortcut lists or
...
- Prefer `SuppressWellKnownBrowserShortcuts(...)` over one-off page-level
  scripts when a WebView app needs to block browser defaults.
...
- Avoid native/global keyboard interception when the issue is only browser DOM
  shortcut behavior inside a Blazor WebView.
...
- `IBlazorWindow`, `IBlazorWindowController`,
  ...
  `LoadScript`, `SuppressWellKnownBrowserShortcuts`, `HeadContent`,
  `ToggleButton`, `IBlazorContentRepository`,
